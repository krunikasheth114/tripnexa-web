import Image from "next/image";
import Link from "next/link";

export default function Footer() {
  return (
    <footer className="bg-[#28536B] text-white">
      <div className="max-w-7xl mx-auto px-6 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          {/* Brand / About */}
          <div className="lg:col-span-1">
            <Link
              href="/"
              className="inline-flex mb-4 rounded-xl bg-white/95 px-3 py-2 shadow-sm ring-1 ring-white/30"
            >
              <Image
                src="/logo-footer.png"
                alt="TripNexa Logo"
                width={150}
                height={80}
                className="h-auto w-[150px]"
              />
            </Link>
            <p className="text-white/70 text-sm leading-relaxed">
              Your next-generation travel partner for exploring Gujarat.
              We simplify travel planning with smart technology and
              personalized support.
            </p>
          </div>

          {/* Quick Links */}
          <div>
            <h3 className="text-sm font-semibold uppercase tracking-widest text-[#C2948A] mb-4">
              Quick Links
            </h3>
            <ul className="space-y-3">
              {[
                { label: "Home", href: "/" },
                { label: "About Us", href: "/contact" },
                { label: "Destinations", href: "/#destinations" },
                { label: "Contact Us", href: "/contact" },
                { label: "Plan a Trip", href: "/about" },
              ].map((link) => (
                <li key={link.label}>
                  <Link
                    href={link.href}
                    className="text-white/70 hover:text-white text-sm transition-colors duration-200"
                  >
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Office Address */}
          <div>
            <h3 className="text-sm font-semibold uppercase tracking-widest text-[#C2948A] mb-4">
              Office Address
            </h3>
            <address className="not-italic text-white/70 text-sm leading-relaxed space-y-3">
              <p>
                TripNexa Travels
                <br />
                Ahmedabad, Gujarat
                <br />
                India
              </p>
              <p>
                <span className="text-white/50 block text-xs uppercase tracking-wide mb-1">
                  Email
                </span>
                <a
                  href="mailto:hello@tripnexa.com"
                  className="hover:text-white transition-colors duration-200"
                >
                  hello@tripnexa.com
                </a>
              </p>
              <p>
                <span className="text-white/50 block text-xs uppercase tracking-wide mb-1">
                  Phone
                </span>
                <a
                  href="tel:+919876543210"
                  className="hover:text-white transition-colors duration-200"
                >
                  +91 98765 43210
                </a>
              </p>
            </address>
          </div>

          {/* Connect */}
          <div>
            <h3 className="text-sm font-semibold uppercase tracking-widest text-[#C2948A] mb-4">
              Connect With Us
            </h3>
            <div className="flex gap-3 mb-6">
              {[
                {
                  label: "Instagram",
                  href: "#",
                  icon: (
                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
                    </svg>
                  ),
                },
                {
                  label: "Facebook",
                  href: "#",
                  icon: (
                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                    </svg>
                  ),
                },
                {
                  label: "Twitter",
                  href: "#",
                  icon: (
                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                    </svg>
                  ),
                },
              ].map((social) => (
                <a
                  key={social.label}
                  href={social.href}
                  aria-label={social.label}
                  className="w-10 h-10 rounded-[8px] bg-white/10 flex items-center justify-center hover:bg-[#C2948A] transition-colors duration-200"
                >
                  {social.icon}
                </a>
              ))}
            </div>
            <p className="text-white/50 text-xs leading-relaxed">
              Follow us for travel tips, destination highlights, and exclusive
              offers across Gujarat.
            </p>
          </div>
        </div>

        {/* Bottom Bar */}
        <div className="mt-12 pt-8 border-t border-white/10 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-white/50 text-xs">
            &copy; {new Date().getFullYear()} TripNexa. All rights reserved.
          </p>
          <div className="flex gap-6">
            <Link
              href="#"
              className="text-white/50 hover:text-white text-xs transition-colors duration-200"
            >
              Privacy Policy
            </Link>
            <Link
              href="#"
              className="text-white/50 hover:text-white text-xs transition-colors duration-200"
            >
              Terms of Service
            </Link>
          </div>
        </div>
      </div>
    </footer>
  );
}
